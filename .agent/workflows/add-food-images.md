...
| 步驟 | 常見問題 | 解決方案 |
|------|----------|----------|
| **圖片路徑** | 檔名不一致 (如 `radish-in-soil` vs `radish-soil`) | 用 `dir` 指令核對實際檔名 |
| **抽卡池** | 新資料沒加入 `items` 陣列 | 確認 `page.tsx` 有展開新資料陣列 |
| **Import** | 忘記 import 新資料檔 | 確認檔案頂部有 import 語句 |
| **WebP 轉換** | 忘記從 PNG 轉換 | 運行 `node convert-to-webp.mjs` |
| **描述段落** | 段落間缺少空行 | 使用 `\n\n` 分隔段落 |
| **check-items-data.js** | 圖片路徑與實際檔名不符 | 逐一核對每個 image 欄位 |
...
const items = [
  ...
  ...newItems,         // #9-50
  ...items51to100,     // #51-100
  ...items101to150,    // #101-150
  ...items151to200     // #151-200 ← 必須加入！
];
```
